...
internal class InlineTableCollector : TableCollectorBase, IConfigurationCollector
{
    public DocumentationTablesModel Collect(INamedTypeSymbol root)
    {
        ...
        List<DocumentationDataModel> leafEntries = [];
        HashSet<INamedTypeSymbol> visited = new(SymbolEqualityComparer.Default);

        Recurse(root, string.Empty, visited, root, leafEntries);

        // Map leaf entries to TableRow in RootRows; keep TypeTables empty for inline mode.
        foreach (DocumentationDataModel e in leafEntries)
        {
            tables.RootRows.Add(new TableRow { Data = e, ComplexTarget = null });
        }
        return tables;
    }
